...
const GENDER_TABS = [
  { key: 'male',   label: '♂ Male',   color: '#60A5FA' },
...
function RankTable({ board, accentColor }) {
  if (!board || board.length === 0) return (
    <div className="rounded-xl p-10 text-center" style={{ background: '#111118', border: '1px solid #1E1E2A' }}>
      <Trophy size={28} className="mx-auto mb-3" style={{ color: '#2A2A3A' }} />
      <p className="text-[13px] font-semibold mb-1" style={{ color: '#4A4A5E' }}>No rankings yet</p>
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ background: '#16161E', border: '1px solid #1E1E2A' }}>
      ...
      <div className="grid px-5 py-3 text-[10px] font-bold uppercase tracking-wider"
        style={{ gridTemplateColumns: '44px 1fr 70px 55px 55px 70px 70px', background: 'rgba(6,11,24,0.5)', borderBottom: '1px solid #1E1E2A', color: '#3A3A52' }}>
      ...
      {board.map((entry, i) => {
        ...
        return (
          <div key={entry._id}
            ...
            style={{ gridTemplateColumns: '44px 1fr 70px 55px 55px 70px 70px', borderBottom: '1px solid #1E1E2A', background: isTop ? `${accentColor}06` : 'transparent' }}
            onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.025)'}
            onMouseLeave={e => e.currentTarget.style.background = isTop ? `${accentColor}06` : 'transparent'}>

            <div className="text-center">
              {isTop ? <span className="text-[18px]">{MEDALS[i]}</span>
            ...
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center text-[13px] font-black flex-shrink-0"
                style={{ background: isTop ? `${accentColor}20` : 'rgba(255,255,255,0.05)', color: isTop ? accentColor : '#4A4A5E', border: isTop ? `1px solid ${accentColor}30` : '1px solid #1E1E2A' }}>
                {entry.name.charAt(0).toUpperCase()}
              </div>
              <div className="min-w-0">
                <p className="text-[13px] font-semibold truncate" style={{ color: '#F4F4F6' }}>{entry.name}</p>
                {entry.players
                  ? <p className="text-[10px] truncate" style={{ color: '#3A3A52' }}>{entry.players.map(p => p.name).join(' & ')}</p>
                  : <p className="text-[10px]" style={{ color: '#3A3A52' }}>{entry.gender === 'male' ? '♂ Male' : '♀ Female'}</p>}
              </div>
            </div>
...
export default function LeaderboardPage() {
  ...
  const accentColor = tab === 'single'
    ? (gender === 'male' ? '#60A5FA' : '#F472B6')
    : activeTab?.color || '#C9A84C';
  ...
  const currentBoard = tab === 'single'
    ? (data.single?.[gender] || [])
    : tab === 'double' ? data.double : data.mixed;
  ...
  return (
    <div className="w-full space-y-5 animate-fade-in">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <h1 className="text-[26px] font-bold mb-1 flex items-center gap-2.5" style={{ color: '#F4F4F6' }}>
            <Trophy size={24} style={{ color: '#C9A84C' }} /> Leaderboard
          </h1>
          <p className="text-[13px]" style={{ color: '#6B8FAD' }}>Ranked by wins, win rate, and total points.</p>
        </div>
        <button onClick={load} className="p-2.5 rounded-xl transition-all mt-1"
      ...
      {/* Gender sub-tabs — only for Singles */}
      {tab === 'single' && (
        <div className="flex gap-2">
          {GENDER_TABS.map(g => {
            const active = gender === g.key;
            const count  = data.single?.[g.key]?.length || 0;
            return (
              <button key={g.key} onClick={() => setGender(g.key)}
                ...
                style={active
                  ? { background: `${g.color}15`, color: g.color, border: `1px solid ${g.color}35`, boxShadow: `0 2px 12px ${g.color}20` }
                  : { background: 'rgba(255,255,255,0.03)', color: '#4A4A5E', border: '1px solid #1E1E2A' }}
                onMouseEnter={e => { if (!active) { e.currentTarget.style.background = 'rgba(255,255,255,0.06)'; e.currentTarget.style.color = '#8B8B9E'; } }}
                onMouseLeave={e => { if (!active) { e.currentTarget.style.background = 'rgba(255,255,255,0.03)'; e.currentTarget.style.color = '#4A4A5E'; } }}>
                {g.label}
                <span className="text-[11px] px-2 py-0.5 rounded-full font-bold"
                  style={{ background: active ? `${g.color}25` : 'rgba(255,255,255,0.05)', color: active ? g.color : '#3A3A52' }}>
                  {count}
                </span>
              </button>
            );
      ...
      {loading ? (
        ...
        <RankTable board={currentBoard} accentColor={accentColor} />
      )}
